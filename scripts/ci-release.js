...
if (TRAVIS_PULL_REQUEST !== 'false') {
    ...
    shell.exec(`curl -s -H "Authorization: token ${BOT_TOKEN}" -X POST -d '{"body": "Thanks for pull request, build for this pr is ready, you can test it with follow builds:\\n- [Build for Windows(Lightproxy-${commitId}.exe)](https://master.dl.sourceforge.net/project/lightproxy/tempbuild/Lightproxy-${commitId}.exe)\\n- [Build for macOS(Lightproxy-${commitId}.dmg)](https://master.dl.sourceforge.net/project/lightproxy/tempbuild/Lightproxy-${commitId}.dmg)"}' "https://api.github.com/repos/alibaba/lightproxy/issues/${TRAVIS_PULL_REQUEST}/comments"`);
}
console.log('done');
